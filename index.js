...
function bsbfAddClient() {
  ...
  fetch(`${config.bsbf_add_client_url}`)
    ...
    .then((text) => {
      ...
      document.getElementById("asu-server-ipv4").value = "102.132.169.219";
      document.getElementById("asu-server-port").value = words[0];
      document.getElementById("asu-uuid").value = words[1];
    })
...
async function buildAsuRequest(request_hash) {
  ...
  /* Run bsbf-client-openwrt-config-generator. */
  if (!request_hash) {
    var packagesParam, defaultsParam;
    const server_ipv4 = $("#asu-server-ipv4").value.trim();
    const server_port = $("#asu-server-port").value.trim();
    const uuid = $("#asu-uuid").value.trim();
    const ocg_args = $("#asu-ocg").value.trim();
    try {
      const ocgResult = await bsbf_client_ocg(server_ipv4, server_port, uuid, ocg_args);
      if (!ocgResult)
        return;
      packagesParam = ocgResult.packagesParam;
      defaultsParam = ocgResult.defaultsParam;
    } catch (error) {
  ...
  if (request_hash) {
    ...
    var body = JSON.stringify({
      ...
      packages: split(packagesParam).concat(split($("#asu-packages").value)),
